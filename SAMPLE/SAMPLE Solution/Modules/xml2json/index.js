...
var xml2json={
	...
	xml_to_object:function(xmlcode){
		...
		for (var i=0;i<y.length;i++){
			...
			tagnamn=escape_element_name(tagnamn);
			//var rest=y[i].split(">")[1];
			var rest=y[i].split(/>/)[1];
			if(niva<=oldniva){
				var tabort=oldniva-niva+1;
				for (var j=0;j<tabort;j++){
					objname=getPath(objname);
...
//added by miyako for cdata support
...
//added by miyako for hyphen in element names
function escape_element_name(tagnamn){
		tagnamn=tagnamn.replace(/%0a|%0d|%09|%20/g, '');
		tagnamn=tagnamn.replace(/%3a/g, ':');
		tagnamn=tagnamn.replace(/%3a|%0a|%0d|%09|%20/g, '');
	return tagnamn;			
}

//added by miyako for colon in element names
...
exports.parse = function(xmlcode){
	if(xmlcode){
		return xml2json.parser(xml_escape(xmlcode));
	}
}